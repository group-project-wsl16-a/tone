...
class Soundboard extends Component {
    ...
    componentDidMount () {
        console.log(window.getComputedStyle(document.getElementById('BeachSynth'), '::-webkit-slider-thumb').getPropertyValue('y-index'));
    }
    ...
    render() {
        ...
        <div className="SoundboardBody" ref={this.SoundboardBody}  >
            ...
            <input ref={this.mountainInput} className="VolumeSlider Blink" id="DrumSlider" type="range" min="-25" max="10" defaultValue='-5' onMouseEnter={() => this.handleMouseEnter('drum')}onInput={(e) => this.handleChange('drum', e.target.value, e.target.min, e.target.max)} />
            {/* Beach Synth */}
            <div className="BeachGrabber" onMouseEnter={() => this.handleMouseEnter('beach')} onMouseLeave={() => this.handleMouseLeave('beach')} ></div>
            <input ref={this.skyInput} className="VolumeSlider Blink" id="BeachSynth" type="range" min="-45" max="6" defaultValue='-10' onInput={(e) => this.handleChange('beach', e.target.value, e.target.min, e.target.max)} />
            <div className="directionalBlinkers BeachBlinkers" ></div>
        </div>
        );
    }
}
